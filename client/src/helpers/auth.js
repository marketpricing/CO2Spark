...
export const TerdaftarLombaEssay = () => {
  if (window !== "undefined") {
    const Essay = LocalUserData().lomba_essay;
    if (Essay === "Belum Terverifikasi" || Essay === "Terverifikasi") {
      return true;
    } else {
      return false;
    } //Kode ini harusnta
...
export const TerdaftarLombaBusiness = () => {
  if (window !== "undefined") {
    const Business = LocalUserData().lomba_business;
    if (Business === "Belum Terverifikasi" || Business === "Terverifikasi") {
      return true;
    } else {
      return false;
    } //Kode ini harusnta
...
export const updateUserFollow = (response, next) => {
  console.log("UPDATE USER IN LOCALSTORAGE HELPERS", response);
  if (typeof window !== "undefined") {
    let auth = JSON.parse(localStorage.getItem("userdata"));
    auth.link_follow_sosmed = response.data; //
    localStorage.setItem("userdata", JSON.stringify(auth));
  }
  next();
};
...
export const updateSubmisiBusiness = (response, next) => {
  console.log("UPDATE USER IN LOCALSTORAGE HELPERS", response);
  if (typeof window !== "undefined") {
    let auth = JSON.parse(localStorage.getItem("userdata"));
    auth.link_submisi_business = response.data; //
    localStorage.setItem("userdata", JSON.stringify(auth));
  }
  next();
};
export const updateSubmisiEssay = (response, next) => {
  console.log("UPDATE USER IN LOCALSTORAGE HELPERS", response);
  if (typeof window !== "undefined") {
    let auth = JSON.parse(localStorage.getItem("userdata"));
    auth.link_submisi_essay= response.data; //
    localStorage.setItem("userdata", JSON.stringify(auth));
  }
  next();
};
...
export const updateBuktiBusiness = (response, next) => {
  console.log("UPDATE USER IN LOCALSTORAGE HELPERS", response);
  if (typeof window !== "undefined") {
    let auth = JSON.parse(localStorage.getItem("userdata"));
    auth.bukti_pembayaran_business = response.data; //
    localStorage.setItem("userdata", JSON.stringify(auth));
  }
  next();
};

export const updateBuktiEssay = (response, next) => {
  console.log("UPDATE USER IN LOCALSTORAGE HELPERS", response);
  if (typeof window !== "undefined") {
    let auth = JSON.parse(localStorage.getItem("userdata"));
    auth.bukti_pembayaran_essay = response.data; //
    localStorage.setItem("userdata", JSON.stringify(auth));
  }
  next();
};
